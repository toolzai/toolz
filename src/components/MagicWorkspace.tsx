...
import { Wand2, ZoomIn, X, RefreshCw, Menu } from "lucide-react";
import clsx from "clsx";

import BackgroundRemover from "./tools/BackgroundRemover";
import ImageUpscaler from "./tools/ImageUpscaler";

export type MagicToolType = "bg-remove" | "upscale" | null;

export default function MagicWorkspace() {
  const [imageFile, setImageFile] = useState<File | null>(null);
  const [imagePreview, setImagePreview] = useState<string | null>(null);
  const [activeTool, setActiveTool] = useState<MagicToolType>(null);
  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    if (acceptedFiles && acceptedFiles.length > 0) {
      ...
      setActiveTool("bg-remove"); // Default tool
    }
  }, []);
    ...
    return (
      <div 
        ...
        className={clsx(
          ...
          isDragActive ? "border-purple-400 bg-purple-50/20" : "border-zinc-200 hover:border-zinc-300"
        )}
      >
        <input {...getInputProps()} />
        ...
        <h3 className="text-2xl font-medium text-zinc-800 mb-2">Drop an image to enchant</h3>
    ...
    <div {...getRootProps()} className="w-full flex flex-col md:flex-row gap-6 h-[700px] transition-all duration-300">
      ...
      <div className="flex-1 min-w-0 glass-panel rounded-3xl p-6 relative flex flex-col transition-all duration-300">
        ...
        <div className="flex-1 w-full relative flex items-center justify-center bg-zinc-50/50 rounded-2xl overflow-hidden">
          {activeTool === "bg-remove" && <BackgroundRemover file={imageFile} previewUrl={imagePreview} />}
          {activeTool === "upscale" && <ImageUpscaler file={imageFile} previewUrl={imagePreview} />}
        </div>
      </div>
      ...
      >
        <div className={clsx("flex items-center justify-between w-full mb-2", !isSidebarExpanded && "justify-center")}>
          {isSidebarExpanded && <h3 className="text-lg font-medium text-zinc-800">Magic Tools</h3>}
          <button 
  ...
  return (
    ...
    >
      <div className={clsx(
        ...
        active 
          ? (isExpanded ? "bg-purple-50 text-purple-600" : "text-purple-600") 
